...
{
    constexpr auto sum(const auto &v)
        -> std::decay_t<decltype(std::begin(v), std::end(v), std::declval<std::enable_if_t<std::is_arithmetic_v<typename std::decay_t<decltype(v)>::value_type>, typename std::decay_t<decltype(v)>::value_type>>())>
    {
        return std::accumulate(std::begin(v), std::end(v), static_cast<typename std::decay_t<decltype(v)>::value_type>(0));
    }
    ...
    constexpr auto linmap(const auto &in_val, const auto &in_min, const auto &in_max, const auto &out_min, const auto &out_max)
    {
